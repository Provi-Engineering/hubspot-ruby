  ...
  class Connection
    ...
    class << self
      def get_json(path, opts)
        ...
        raise(CommunityHubspot::RequestError.new(response)) unless response.success?
      ...
      def read_timeout(opts = {})
        opts.delete(:read_timeout) || CommunityHubspot::Config.read_timeout
      end

      def open_timeout(opts = {})
        opts.delete(:open_timeout) || CommunityHubspot::Config.open_timeout
      end
      ...
      def handle_response(response)
        if response.success?
          ...
          raise(CommunityHubspot::RequestError.new(response))
        end
      end

      def log_request_and_response(uri, response, body=nil)
        CommunityHubspot::Config.logger.info "CommunityHubspot: #{uri}.\nBody: #{body}.\nResponse: #{response.code} #{response.body}"
      end

      def generate_url(path, params={}, options={})
        if CommunityHubspot::Config.access_token.present?
          options[:hapikey] = false
        else
          CommunityHubspot::Config.ensure! :hapikey
        end
        path = path.clone
        params = params.clone
        base_url = options[:base_url] || CommunityHubspot::Config.base_url
        params["hapikey"] = CommunityHubspot::Config.hapikey unless options[:hapikey] == false

        if path =~ /:portal_id/
          CommunityHubspot::Config.ensure! :portal_id
          params["portal_id"] = CommunityHubspot::Config.portal_id if path =~ /:portal_id/
        end
        ...
        raise(CommunityHubspot::MissingInterpolation.new("Interpolation not resolved")) if path =~ /:/
