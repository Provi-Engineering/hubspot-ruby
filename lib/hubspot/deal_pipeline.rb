  ...
  class DealPipeline
    ...
    class << self
      def find(pipeline_id)
        response = CommunityHubspot::Connection.get_json(PIPELINE_PATH, { pipeline_id: pipeline_id })
        new(response)
      end

      def all
        response = CommunityHubspot::Connection.get_json(PIPELINES_PATH, {})
        response.map { |p| new(p) }
      end

      # Creates a DealPipeline
      # {https://developers.hubspot.com/docs/methods/deal-pipelines/create-deal-pipeline}
      # @return [CommunityHubspot::PipeLine] Company record
      def create!(post_data={})
        response = CommunityHubspot::Connection.post_json(PIPELINES_PATH, params: {}, body: post_data)
        new(response)
      end
    end
    ...
    def destroy!
      CommunityHubspot::Connection.delete_json(PIPELINE_PATH, pipeline_id: @pipeline_id)
    end
