  ...
  class ContactList
    ...
    class << self
      ...
      def create!(opts={})
        ...
        portal_id = opts.delete(:portal_id) { CommunityHubspot::Config.portal_id }

        response = CommunityHubspot::Connection.post_json(LISTS_PATH, params: {}, body: opts.merge({ dynamic: dynamic, portal_id: portal_id}) )
        new(response)
      end
    ...
    def update!(opts={})
      response = CommunityHubspot::Connection.post_json(LIST_PATH, params: { list_id: @id }, body: opts)
    ...
    def destroy!
      response = CommunityHubspot::Connection.delete_json(LIST_PATH, { list_id: @id })
    ...
    def contacts(opts={})
      ...
      if bypass_cache || @contacts.nil?
        ...
        response = CommunityHubspot::Connection.get_json(path, CommunityHubspot::ContactProperties.add_default_parameters(opts))
        @contacts = response['contacts'].map! { |c| CommunityHubspot::Contact.new(c) }
    ...
    def refresh
      response = CommunityHubspot::Connection.post_json(REFRESH_PATH, params: { list_id: @id, no_parse: true }, body: {})
      response.code == 204
    end
    ...
    def add(contacts)
      ...
      response = CommunityHubspot::Connection.post_json(ADD_CONTACT_PATH, params: { list_id: @id }, body: { vids: contact_ids })
    ...
    def remove(contacts)
      contact_ids = [contacts].flatten.uniq.compact.map(&:vid)
      response = CommunityHubspot::Connection.post_json(REMOVE_CONTACT_PATH, params: { list_id: @id }, body: { vids: contact_ids })
      response['updated'].sort == contact_ids.sort
    end
