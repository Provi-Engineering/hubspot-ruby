  ...
  class Deal
    ...
    class << self
      def create!(portal_id, company_ids, vids, params={})
        #TODO: clean following hash, CommunityHubspot::Utils should do the trick
        associations_hash = {"portalId" => portal_id, "associations" => { "associatedCompanyIds" => company_ids, "associatedVids" => vids}}
        post_data = associations_hash.merge({ properties: CommunityHubspot::Utils.hash_to_properties(params, key_name: "name") })

        response = CommunityHubspot::Connection.post_json(CREATE_DEAL_PATH, params: {}, body: post_data )
        new(response)
      end
       ...
       # Usage
       # CommunityHubspot::Deal.associate!(45146940, [], [52])
       def associate!(deal_id, company_ids=[], vids=[])
         ...
         CommunityHubspot::Connection.put_json(ASSOCIATE_DEAL_PATH, params: { deal_id: deal_id, OBJECTTYPE: objecttype, objectId: object_ids}, body: {})
       end


      def find(deal_id)
        response = CommunityHubspot::Connection.get_json(DEAL_PATH, { deal_id: deal_id })
        new(response)
      end
      ...
      def recent(opts = {})
        response = CommunityHubspot::Connection.get_json(RECENT_UPDATED_PATH, opts)
      ...
      # @param company [CommunityHubspot::Company] the company
      # @return [Array] Array of CommunityHubspot::Deal records
      ...
      # {http://developers.hubspot.com/docs/methods/deals/get-associated-deals}
      # @param contact [CommunityHubspot::Contact] the contact
      # @return [Array] Array of CommunityHubspot::Deal records
      ...
      # {http://developers.hubspot.com/docs/methods/deals/get-associated-deals}
      # @param object [CommunityHubspot::Contact || CommunityHubspot::Company] a contact or company
      # @return [Array] Array of CommunityHubspot::Deal records
      def find_by_association(object)
        path = ASSOCIATED_DEAL_PATH
        objectType =  case object
                      when CommunityHubspot::Company then :company
                      when CommunityHubspot::Contact then :contact
                      else raise(CommunityHubspot::InvalidParams, "Instance type not supported")
                      end
    ...
    def destroy!
      CommunityHubspot::Connection.delete_json(DEAL_PATH, {deal_id: deal_id})
      @destroyed = true
    end
    ...
    # @return [CommunityHubspot::Deal] self
    def update!(params)
      query = {"properties" => CommunityHubspot::Utils.hash_to_properties(params.stringify_keys!, key_name: 'name')}
      CommunityHubspot::Connection.put_json(UPDATE_DEAL_PATH, params: { deal_id: deal_id }, body: query)
