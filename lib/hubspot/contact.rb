  ...
  class Contact
    ...
    class << self
      ...
      def create!(email, params={})
        ...
        post_data = {properties: CommunityHubspot::Utils.hash_to_properties(params_with_email)}
        response = CommunityHubspot::Connection.post_json(CREATE_CONTACT_PATH, params: {}, body: post_data )
        new(response)
      end
      ...
      def all(opts={})
        ...
        path, opts =
        if recent_created
          [RECENTLY_CREATED_PATH, CommunityHubspot::ContactProperties.add_default_parameters(opts)]
        elsif recent
          [RECENTLY_UPDATED_PATH, CommunityHubspot::ContactProperties.add_default_parameters(opts)]
        else
          [CONTACTS_PATH, opts]
        end
      ...
      def createOrUpdate(email, params={})
        post_data = {properties: CommunityHubspot::Utils.hash_to_properties(params.stringify_keys)}
        response = CommunityHubspot::Connection.post_json(CREATE_OR_UPDATE_PATH, params: { contact_email: email }, body: post_data )
        ...
        query = contacts.map do |ch|
          ...
          if contact_hash[:vid]
            contact_param = {
              ...
              properties: CommunityHubspot::Utils.hash_to_properties(contact_hash.except(:vid))
            }
          elsif contact_hash[:email]
            contact_param = {
              email: contact_hash[:email],
              properties: CommunityHubspot::Utils.hash_to_properties(contact_hash.except(:email))
            }
          else
            raise CommunityHubspot::InvalidParams, 'expecting vid or email for contact'
          end
          contact_param
        end
        CommunityHubspot::Connection.post_json(BATCH_CREATE_OR_UPDATE_PATH,
                                      params: {},
                                      body: query)
      end
      ...
      def find_by_id(vids)
        ...
        else raise CommunityHubspot::InvalidParams, 'expecting Integer or Array of Integers parameter'
        end

        response = CommunityHubspot::Connection.get_json(path, params)
        raise CommunityHubspot::ApiError if batch_mode
        new(response)
      end
      ...
      def find_by_email(emails)
        ...
        else raise CommunityHubspot::InvalidParams, 'expecting String or Array of Strings parameter'
        end
      ...
      def search(query, options = {})
        ...
        response = CommunityHubspot::Connection.get_json(QUERY_PATH, { q: query, count: count, offset: offset })
      ...
      def merge!(primary_contact_vid, secondary_contact_vid)
        CommunityHubspot::Connection.post_json(
          MERGE_CONTACT_PATH,
    ...
    # @return [CommunityHubspot::Contact] self
    def update!(params)
      query = {"properties" => CommunityHubspot::Utils.hash_to_properties(params.stringify_keys!)}
      CommunityHubspot::Connection.post_json(UPDATE_CONTACT_PATH, params: { contact_id: vid }, body: query)
    ...
    def destroy!
      CommunityHubspot::Connection.delete_json(DESTROY_CONTACT_PATH, { contact_id: vid })
      @destroyed = true
    end
