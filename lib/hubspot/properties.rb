  ...
  class Properties
    ...
    class << self
      ...
      def create!(path, params={})
        ...
        CommunityHubspot::Connection.post_json(path, params: {}, body: post_data)
      end

      def update!(path, property_name, params={})
        post_data = valid_property_params(params)
        return nil if post_data.blank?
        CommunityHubspot::Connection.put_json(path, params: { property_name: property_name }, body: post_data)
      end

      def delete!(path, property_name)
        response = CommunityHubspot::Connection.delete_json(path, property_name: property_name)
      ...
      def update_group!(path, group_name, params={})
        post_data = valid_group_params(params)
        return nil if post_data.blank?
        CommunityHubspot::Connection.put_json(path, params: { group_name: group_name }, body: post_data)
      end

      def delete_group!(path, group_name)
        response = CommunityHubspot::Connection.delete_json(path, group_name: group_name)
        response.parsed_response
      end
