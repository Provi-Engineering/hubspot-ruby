module CommunityHubspot
  ...
  class Company
    ...
    class << self
      ...
      # @return [Array] Array of CommunityHubspot::Company records
      def all(opts={})
        ...
        response = CommunityHubspot::Connection.get_json(path, opts)
      ...
      def find_by_domain(domain, options = {})
        raise CommunityHubspot::InvalidParams, 'expecting String parameter' unless domain.try(:is_a?, String)

        limit = options.fetch(:limit, 100)
        properties = options.fetch(:properties) { CommunityHubspot::CompanyProperties.all.map { |property| property["name"] } }
        ...
        companies = []
        begin
          response = CommunityHubspot::Connection.post_json(GET_COMPANY_BY_DOMAIN_PATH, params: { domain: domain }, body: post_data )
      ...
      # @return [CommunityHubspot::Company] Company record
      def find_by_id(id)
        path = GET_COMPANY_BY_ID_PATH
        params = { company_id: id }
        raise CommunityHubspot::InvalidParams, 'expecting Integer parameter' unless id.try(:is_a?, Integer)
        response = CommunityHubspot::Connection.get_json(path, params)
        new(response)
      end
      ...
      def create!(name, params={})
        ...
        post_data = {properties: CommunityHubspot::Utils.hash_to_properties(params_with_name, key_name: "name")}
        response = CommunityHubspot::Connection.post_json(CREATE_COMPANY_PATH, params: {}, body: post_data )
        new(response)
      end
        ...
        query = companies.map do |company|
          ...
          if company_hash[:vid]
            ...
            company_param = {
              ...
              properties: CommunityHubspot::Utils.hash_to_properties(company_hash.except(:vid).stringify_keys!, key_name: 'name'),
            }
          elsif company_hash[:objectId]
            company_param = {
              objectId: company_hash[:objectId],
              properties: CommunityHubspot::Utils.hash_to_properties(company_hash.except(:objectId).stringify_keys!, key_name: 'name'),
            }
          else
            raise CommunityHubspot::InvalidParams, 'expecting vid or objectId for company'
          end
          company_param
        end
        CommunityHubspot::Connection.post_json(BATCH_UPDATE_PATH, params: {}, body: query)
      end
      ...
      def add_contact!(company_vid, contact_vid)
        CommunityHubspot::Connection.put_json(ADD_CONTACT_TO_COMPANY_PATH,
                                     params: {
    ...
    def initialize(response_hash)
      ...
      @properties = CommunityHubspot::Utils.properties_to_hash(props)
    ...
    # @return [CommunityHubspot::Company] self
    def update!(params)
      query = {"properties" => CommunityHubspot::Utils.hash_to_properties(params.stringify_keys!, key_name: "name")}
      CommunityHubspot::Connection.put_json(UPDATE_COMPANY_PATH, params: { company_id: vid }, body: query)
    ...
    def get_contact_vids
      ...
      vids = []
      loop do
        data = CommunityHubspot::Connection.get_json(GET_COMPANY_CONTACT_VIDS_PATH,
                                            company_id: vid,
    ...
    def add_contact(contact_or_vid)
      contact_vid = if contact_or_vid.is_a?(CommunityHubspot::Contact)
                      contact_or_vid.vid
                    else
                      contact_or_vid
                    end
    ...
    def destroy!
      CommunityHubspot::Connection.delete_json(DESTROY_COMPANY_PATH, { company_id: vid })
      @destroyed = true
    end
    ...
    # @return [Array] Array of CommunityHubspot::Contact records
    def contacts
      response = CommunityHubspot::Connection.get_json(GET_COMPANY_CONTACTS_PATH, company_id: vid)
      response['contacts'].each_with_object([]) do |contact, memo|
        memo << CommunityHubspot::Contact.find_by_id(contact['vid'])
      end
    end
  end
end
