module CommunityHubspot
  ...
  class Blog
    ...
    class << self
      ...
      # @return [CommunityHubspot::Blog] the first 20 blogs or empty_array
      def list
        response = CommunityHubspot::Connection.get_json(BLOG_LIST_PATH, {})
      ...
      # @return CommunityHubspot::Blog
      def find_by_id(id)
        response = CommunityHubspot::Connection.get_json(GET_BLOG_BY_ID_PATH, { blog_id: id })
        new(response)
      end
    end
    ...
    # @return [CommunityHubspot::BlogPost]
    def posts(params = {})
      default_params = {
      ...
      raise CommunityHubspot::InvalidParams.new('params must be passed as a hash') unless params.is_a?(Hash)
      params = default_params.merge(params)

      raise CommunityHubspot::InvalidParams.new('State parameter was invalid') unless [false, 'PUBLISHED', 'DRAFT'].include?(params[:state])
      params.each { |k, v| params.delete(k) if v == false }

      response = CommunityHubspot::Connection.get_json(BLOG_POSTS_PATH, params)
  ...
  class BlogPost
    ...
    def self.find_by_blog_post_id(id)
      response = CommunityHubspot::Connection.get_json(GET_BLOG_POST_BY_ID_PATH, { blog_post_id: id })
      new(response)
    end
      ...
      @topics ||= begin
        if @properties['topic_ids'].empty?
          ...
          @properties['topic_ids'].map do |topic_id|
            CommunityHubspot::Topic.find_by_topic_id(topic_id)
          end
        end
      end
    end
