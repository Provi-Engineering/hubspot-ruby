...
module CommunityHubspot
  ...
  class Engagement
    ...
    class << self
      def create!(params={})
        response = CommunityHubspot::Connection.post_json(CREATE_ENGAGMEMENT_PATH, params: {}, body: params )
        new(HashWithIndifferentAccess.new(response))
      end

      def find(engagement_id)
        begin
          response = CommunityHubspot::Connection.get_json(ENGAGEMENT_PATH, { engagement_id: engagement_id })
          response ? new(HashWithIndifferentAccess.new(response)) : nil
        rescue CommunityHubspot::RequestError => ex
      ...
      def find_by_association(association_id, association_type)
        ...
        raise CommunityHubspot::InvalidParams, 'expecting Integer parameter' unless association_id.try(:is_a?, Integer)
        raise CommunityHubspot::InvalidParams, 'expecting String parameter' unless association_type.try(:is_a?, String)
      ...
      def associate!(engagement_id, object_type, object_vid)
        CommunityHubspot::Connection.put_json(ASSOCIATE_ENGAGEMENT_PATH,
                                     params: {
    ...
    def destroy!
      CommunityHubspot::Connection.delete_json(ENGAGEMENT_PATH, {engagement_id: id})
      @destroyed = true
    end
    ...
    # @return [CommunityHubspot::Engagement] self
    def update!(params)
      data = {
      ...
      CommunityHubspot::Connection.put_json(ENGAGEMENT_PATH, params: { engagement_id: id }, body: data)
      self
    end
  end
