...
namespace :hubspot do
  ...
  task :dump_properties, [:kind, :file, :hapikey, :include, :exclude] do |_, args|
    CommunityHubspot::Deprecator.build.deprecation_warning("hubspot:dump_properties")
    ...
    klass = kind == 'contact' ? CommunityHubspot::ContactProperties : CommunityHubspot::DealProperties
    props = CommunityHubspot::Utils::dump_properties(klass, hapikey, build_filter(args))
    if args[:file].blank?
  ...
  task :restore_properties, [:kind, :file, :hapikey, :dry_run] do |_, args|
    CommunityHubspot::Deprecator.build.deprecation_warning("hubspot:restore_properties")
    ...
    CommunityHubspot::Utils.restore_properties(klass, hapikey, props, args[:dry_run] != 'false')
  end
