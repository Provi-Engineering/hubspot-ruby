describe CommunityHubspot::ContactList do
  ...
  let(:static_list) { CommunityHubspot::ContactList.all(static: true, count: 3).last }
  let(:dynamic_list) { CommunityHubspot::ContactList.all(dynamic: true, count: 1).first }
  ...
  describe '#initialize' do
    subject { CommunityHubspot::ContactList.new(example_contact_list_hash) }

    it { should be_an_instance_of CommunityHubspot::ContactList }
    ...
    it 'has the correct properties' do
      ...
      expect(subject.portal_id).to be_an(Integer)
  ...
  describe '#contacts' do
    ...
    let(:list) { CommunityHubspot::ContactList.new(example_contact_list_hash) }
    ...
    expect_count_and_offset do |params|
      CommunityHubspot::ContactList.find(1).contacts(params)
    end
  end

  describe '.create' do
    subject{ CommunityHubspot::ContactList.create!({ name: name }) }
    ...
    context 'with all required parameters' do
      ...
      context 'adding filters parameters' do
        ...
        it 'returns a ContactList object with filters set' do
          ...
          list_with_filters = CommunityHubspot::ContactList.create!({ name: name, filters: filters_param })
          expect(list_with_filters).to be_a(CommunityHubspot::ContactList)
    ...
    context 'without all required parameters' do
      ...
      it 'raises an error' do
        expect { CommunityHubspot::ContactList.create!({ name: nil }) }.to raise_error(CommunityHubspot::RequestError)
      end
    end
  end
  ...
  describe '.all' do
    context 'all list types' do
      ...
      it 'returns by default 20 contact lists' do
        lists = CommunityHubspot::ContactList.all
        expect(lists.count).to eql 20

        list = lists.first
        expect(list).to be_a(CommunityHubspot::ContactList)
        expect(list.id).to be_an(Integer)
      end

      expect_count_and_offset { |params| CommunityHubspot::ContactList.all(params) }
    end
    ...
    context 'static lists' do
      ...
      it 'returns by defaut all the static contact lists' do
        lists = CommunityHubspot::ContactList.all(static: true)
    ...
    context 'dynamic lists' do
      ...
      it 'returns by defaut all the static contact lists' do
        lists = CommunityHubspot::ContactList.all(dynamic: true)
        expect(lists.count).to be > 20
  ...
  describe '.find' do
    context 'given an id' do
      ...
      subject { CommunityHubspot::ContactList.find(id) }
      ...
      context 'when the contact list is found' do
        ...
        it 'has the correct property' do
          expect(subject.name).to eq('twitterers')
        end
        let(:id) { '1' }
        it { should be_an_instance_of CommunityHubspot::ContactList }
      end
      ...
      context 'Wrong parameter type given' do
        it 'raises an error' do
          expect { CommunityHubspot::ContactList.find(static_list) }.to raise_error(CommunityHubspot::InvalidParams)
        end
      end

      context 'when the contact list is not found' do
        it 'raises an error' do
          expect { CommunityHubspot::ContactList.find(-1) }.to raise_error(CommunityHubspot::RequestError)
        end
      end
    end
    ...
    context 'given a list of ids' do
      ...
      it 'find lists of contacts' do
        lists = CommunityHubspot::ContactList.find([2,3,4])
        list = lists.first
        expect(list).to be_a(CommunityHubspot::ContactList)
  ...
  describe '#add' do
    ...
    context 'static list' do
      it 'returns true if contacts have been added to the list' do
        contact = CommunityHubspot::Contact.all(count: 1).first
        # mock(CommunityHubspot::Connection).post_json("/contacts/v1/lists/:list_id/add", {:params=>{:list_id=>4}, :body=>{:vids=>[contact.vid]}}) { { 'updated' => [contact.vid] } }
    ...
    context 'dynamic list' do
      it 'raises error if try to add a contact to a dynamic list' do
        contact = CommunityHubspot::Contact.new(example_contact_hash)
        expect { dynamic_list.add(contact) }.to raise_error(CommunityHubspot::RequestError)
      end
    end
  end
  ...
  describe '#remove' do
    ...
    context 'static list' do
      ...
      it 'returns false if the contact cannot be removed' do
        contact_not_present_in_list = CommunityHubspot::Contact.new(example_contact_hash)
    ...
    context 'dynamic list' do
      it 'raises error if try to remove a contact from a dynamic list' do
        ...
        expect { dynamic_list.remove(contact) }.to raise_error(CommunityHubspot::RequestError)
      end
    end
  end
  ...
  describe '#update!' do
    ...
    let(:contact_list) { CommunityHubspot::ContactList.new(example_contact_list_hash) }
    ...
    it 'has the correct property' do
      expect(subject.name).to eq("update list name")
    end
  end
  ...
  describe '#destroy!' do
    ...
    let(:contact_list) { CommunityHubspot::ContactList.create!({ name: "newcontactlist_#{Time.now.to_i}"}) }
    subject{ contact_list.destroy! }
    it { should be_truthy }

    it "should be destroyed" do
      subject
      contact_list.destroyed?.should be_truthy
    end
  end
