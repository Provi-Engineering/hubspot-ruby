describe CommunityHubspot::Connection do
  before(:each) do
  ...
  describe '.get_json' do
    it 'delegates url format to CommunityHubspot::Utils, call HTTParty get and returns response' do
      # @http_response.success? { true }
      # @http_response.parsed_response { {} }
      # @http_response.code { 200 }
      # @http_response.body { 'mocked response' }

      # mock(CommunityHubspot::Connection).generate_url(@url, {}) { @url }
      # mock(CommunityHubspot::Connection).get(@url, format: :json, read_timeout: nil, open_timeout: nil) { @http_response }
      # CommunityHubspot::Connection.get_json(@url, {})
    end
  end

  describe '.post_json' do
    it 'delegates url format to CommunityHubspot::Utils, call HTTParty post and returns response' do
      # @http_response.success? { true }
      # @http_response.parsed_response { {} }
      # @http_response.code { 200 }
      # @http_response.body { 'mocked response' }
      ...
      # mock(CommunityHubspot::Connection).generate_url(@url, {}) { @url }
      # mock(CommunityHubspot::Connection).post(@url, body: "{}", headers: {"Content-Type"=>"application/json"}, format: :json, read_timeout: nil, open_timeout: nil) { @http_response }
      # CommunityHubspot::Connection.post_json(@url, params: {}, body: {})
    end
  end

  describe '.delete_json' do
    it 'delegates url format to CommunityHubspot::Utils, call HTTParty delete and returns response' do
      # @http_response.success? { true }
      # @http_response.code { 200 }
      # @http_response.body { 'mocked response' }
      ...
      # skip
      # mock(CommunityHubspot::Connection).generate_url(@url, {}) { @url }
      # mock(CommunityHubspot::Connection).delete(@url, format: :json, read_timeout: nil, open_timeout: nil) { @http_response }
      # CommunityHubspot::Connection.delete_json(@url, {})
    end
  end
  ...
  context 'private methods' do
    describe ".generate_url" do
      ...
      let(:options){{}}
      subject{ CommunityHubspot::Connection.send(:generate_url, path, params, options) }
      ...
      context "when configure hasn't been called" do
        before{ CommunityHubspot::Config.reset! }
      ...
      context "with interpolations but no params" do
        ...
        it "raises an interpolation exception" do
          expect{ subject }.to raise_error CommunityHubspot::MissingInterpolation
        end
      end
      ...
      context "passing Array as parameters for batch mode, key is prefixed with batch_" do
        let(:path) { CommunityHubspot::ContactList::LIST_BATCH_PATH }
