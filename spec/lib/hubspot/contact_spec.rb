RSpec.describe CommunityHubspot::Contact do
  ...
  describe '#initialize' do
    subject{ CommunityHubspot::Contact.new(example_contact_hash) }
    it{ should be_an_instance_of CommunityHubspot::Contact }
    ...
    it 'has correct properties' do
      expect(subject.utk).to eq('1234567890')
      expect(subject.vid).to eq(82325)
      expect(subject['email']).to eq('testingapis@hubspot.com')
      expect(subject['firstname']).to eq('Clint')
      expect(subject['lastname']).to eq('Eastwood')
      expect(subject['phone']).to eq('555-555-5432')
    end
  end
  ...
  describe '.create!' do
    ...
    let(:params){{}}
    subject{ CommunityHubspot::Contact.create!(email, params) }
    context 'with a new email' do
      let(:email){ "newcontact#{Time.now.to_i}@hsgem.com" }
      it{ should be_an_instance_of CommunityHubspot::Contact }
      it 'has correct properties' do
        expect(subject.email).to match(/newcontact.*@hsgem.com/)
      end
      ...
      context 'and some params' do
        ...
        it 'has correct properties' do
          expect(subject['firstname']).to eq('Hugh')
          expect(subject['lastname']).to eq('Jackman')
        end
      end
    end
    ...
    context "when the contact already exists" do
      it "updates the contact" do
        VCR.use_cassette("contacts/update_contact") do
          existing_contact = CommunityHubspot::Contact.create!("contact@example.com")
          email = existing_contact.email
          new_email = "new_email@example.com"
          params = { email: new_email }

          contact = CommunityHubspot::Contact.createOrUpdate(email, params)

          assert_requested :post, hubspot_api_url("/contacts/v1/contact/createOrUpdate/email/#{email}?hapikey=demo")
          expect(contact).to be_a(CommunityHubspot::Contact)
    ...
    context "with an invalid email" do
      it "raises an error" do
        VCR.use_cassette("contact_create_or_update_invalid_email") do
          expect {
            CommunityHubspot::Contact.createOrUpdate("not_an_email")
          }.to raise_error(CommunityHubspot::RequestError)
        end
      end
    end
  end
  ...
  describe '.create_or_update!' do
    ...
    let(:existing_contact) do
      CommunityHubspot::Contact.create!("morpheus@example.com", firstname: 'Morpheus')
    end
    before do
      CommunityHubspot::Contact.create_or_update!(params)
    end
    ...
    it 'creates and updates contacts' do
      contact = CommunityHubspot::Contact.find_by_id existing_contact.vid
      expect(contact.properties['firstname']).to eql 'Neo'
      latest_contact_email = CommunityHubspot::Contact.all(recent: true).first.email
      new_contact = CommunityHubspot::Contact.find_by_email(latest_contact_email)
  ...
  describe '.find_by_email' do
    context 'given an uniq email' do
      ...
      subject{ CommunityHubspot::Contact.find_by_email(email) }
      ...
      context 'when the contact cannot be found' do
        it 'raises an error' do
          expect { CommunityHubspot::Contact.find_by_email('notacontact@test.com') }.to raise_error(CommunityHubspot::RequestError)
        end
      end
    end
    ...
    context 'batch mode' do
      ...
      it 'find lists of contacts' do
        ...
        contacts = CommunityHubspot::Contact.find_by_email(emails)
      end
    end
  end
  ...
  describe '.find_by_id' do
    before do
      @s = StringIO.new
      CommunityHubspot::Config.logger = Logger.new(@s)
    end

    context 'given an uniq id' do
      cassette 'contact_find_by_id'
      subject{ CommunityHubspot::Contact.find_by_id(vid) }
      ...
      context 'when the contact cannot be found' do
        it 'raises an error' do
          expect { CommunityHubspot::Contact.find_by_id(9999999) }.to raise_error(CommunityHubspot::RequestError)
    ...
    context 'batch mode' do
      ...
      it 'find lists of contacts' do
        expect { CommunityHubspot::Contact.find_by_id([82325]) }.to raise_error(CommunityHubspot::ApiError)
  ...
  describe '.find_by_utk' do
    context 'given an uniq utk' do
      ...
      subject{ CommunityHubspot::Contact.find_by_utk(utk) }

      context 'when the contact is found' do
        let(:utk){ 'f844d2217850188692f2610c717c2e9b' }
        it{ should be_an_instance_of CommunityHubspot::Contact }
        it 'has correct properties' do
          expect(subject.utk).to eq('f844d2217850188692f2610c717c2e9b')
        end
      ...
      context 'when the contact cannot be found' do
        it 'raises an error' do
          expect { CommunityHubspot::Contact.find_by_utk('invalid') }.to raise_error(CommunityHubspot::RequestError)
        end
      end
    end
    ...
    context 'batch mode' do
      ...
      it 'find lists of contacts' do
        ...
        expect { CommunityHubspot::Contact.find_by_utk(utks) }.to raise_error(CommunityHubspot::ApiError)
      end
    end
  end

  describe '.search' do
    subject { CommunityHubspot::Contact.search(query, options) }
    ...
    context 'when query returns contacts' do
      ...
      it { subject['contacts'].each { |contact| expect(contact).to be_an_instance_of(CommunityHubspot::Contact) } }
  ...
  describe '.all' do
    context 'all contacts' do
      ...
      it 'must get the contacts list' do
        contacts = CommunityHubspot::Contact.all
      ...
      it 'must get the contacts list with paging data' do
        contact_data = CommunityHubspot::Contact.all({paged: true})
      ...
      it 'must filter only 2 contacts' do
        contacts = CommunityHubspot::Contact.all(count: 2)
        expect(contacts.size).to eql 2
      end

      it 'it must offset the contacts' do
        single_list = CommunityHubspot::Contact.all(count: 1)
        expect(single_list.size).to eql 1
        first = single_list.first

        second = CommunityHubspot::Contact.all(count: 1, vidOffset: first.vid).first
    ...
    context 'recent contacts' do
      ...
      it 'must get the contacts list' do
        contacts = CommunityHubspot::Contact.all(recent: true)
    ...
    context 'recent created contacts' do
      ...
      it 'must get the contacts list' do
        contacts = CommunityHubspot::Contact.all(recent_created: true)
        expect(contacts.size).to eql 20
  ...
  describe '.merge!' do
    ...
    let(:primary_contact) { CommunityHubspot::Contact.create!("primary_#{Time.now.to_i}@hsgem.com", primary_params) }
    let(:secondary_params) { { firstname: 'Wolverine' } }
    let(:secondary_contact) { CommunityHubspot::Contact.create!("secondary_#{Time.now.to_i}@hsgem.com", secondary_params) }

    subject { CommunityHubspot::Contact.merge!(primary_contact.vid, secondary_contact.vid) }

    it 'merges the contacts' do
      subject

      primary_find_by_id = CommunityHubspot::Contact.find_by_id primary_contact.vid
      primary_find_by_email = CommunityHubspot::Contact.find_by_email primary_contact.email
      secondary_find_by_id = CommunityHubspot::Contact.find_by_id secondary_contact.vid
      secondary_find_by_email = CommunityHubspot::Contact.find_by_email secondary_contact.email
  ...
  describe '#update!' do
    ...
    let(:contact){ CommunityHubspot::Contact.new(example_contact_hash) }
    let(:params){ {firstname: 'Steve', lastname: 'Cunningham'} }
    subject{ contact.update!(params) }

    it{ should be_an_instance_of CommunityHubspot::Contact }
    it 'has correct properties' do
      expect(subject['firstname']).to eq('Steve')
      expect(subject['lastname']).to eq('Cunningham')
    end
    # its(['firstname']){ should ==  'Steve' }
    # its(['lastname']){ should ==  'Cunningham' }

    context 'when the request is not successful' do
      let(:contact){ CommunityHubspot::Contact.new({'vid' => 'invalid', 'properties' => {}})}
      it 'raises an error' do
        expect{ subject }.to raise_error CommunityHubspot::RequestError
      end
    end
  ...
  describe '#destroy!' do
    ...
    let(:contact){ CommunityHubspot::Contact.create!("newcontact_y_#{Time.now.to_i}@hsgem.com") }
    subject{ contact.destroy! }
    it { should be_truthy }
    it 'should be destroyed' do
      subject
      contact.destroyed?.should be_truthy
    end
    context 'when the request is not successful' do
      let(:contact){ CommunityHubspot::Contact.new({'vid' => 'invalid', 'properties' => {}})}
      it 'raises an error' do
        expect{ subject }.to raise_error CommunityHubspot::RequestError
        contact.destroyed?.should be_falsey
      end
    end
  end
